...
class LogExport extends Backend
{
    ...
    public function exportLog( )
    {
        ...
        $datei = $rootDir . '/system/tmp/Zeitnachweis_' . StringUtil::prepareSlug( $GLOBALS['TIMETRACKER']['KUNDEN'][$kundeID]['kundenname'], $kundeID ) . '_' . date( 'my' ) . '.xlsx';
        ...
        $sheet->setCellValue( 'C' . $line, 'Summe (entspricht ' . str_replace( '.', ',', sprintf( '%2.1f', round( $summe['berechnet'] / 3600, 1 ) ) ) . ' Stunden)' );
    ...
    {
        if( Input::get( 'key' ) != 'opentimes' ) return '';
